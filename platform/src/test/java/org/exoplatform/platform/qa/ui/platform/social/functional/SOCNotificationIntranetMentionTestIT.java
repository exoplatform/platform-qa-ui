...
public class SOCNotificationIntranetMentionTestIT extends Base {
  ...
  /*
   * Step Number: 1 Step Name: Step Description: - Login with User B - Click the
   * notification icons in the top navigation - Check the notification list Input
   * Data: Expected Outcome: - The Mention notification is displayed in the list
   */
  /*
   * Step number: 2 Step Name: Step Description: - Check the notification message
   * Input Data: Expected Outcome: The notification message is : $AVATAR$USER has
   * mentioned you $ACTIVITY$DATEWhere : - $AVATAR is the thumbnail of User A -
   * $USER is User A - $ACTIVITY is the activity title/message - $DATE is the date
   * of the notification
   */
  /*
   * Step Number: 1 Step Name: Step Description: - Login with User B - Click the
  ...
  /*
   * Step number: 2 Step Name: Step Description: - Click the notification Input
   * Data: Expected Outcome: - The user is redirected to the activity viewer with
   * all comment expanded.
   */
  @Test
  ...
  public void test04_CheckMentionNotificationInActivityMessage() {
    ...
    intranetNotification.checkActivityTitleInStatus(activity1, true);
    refresh();
    $(byXpath(ELEMENT_ACTIVITY_MENTION_USER.replace("${content}", activity1)
                                           .replace("${user}", username2))).waitUntil(Condition.visible, Configuration.timeout);
    manageLogInOut.signIn(DATA_USER1, "gtngtn");
    navigationToolbar.goToManageCommunity();
    addUsers.deleteUser(username1);
    addUsers.deleteUser(username2);
   ...
   * Input Data: Expected Outcome: The notification message is : $AVATAR$USER has
   * mentioned you $ACTIVITY$DATEWhere : - $AVATAR is the thumbnail of User A -
   * $USER is User A - $ACTIVITY is the name of the wiki page - $DATE is the date
   * of the notification
   */
  @Test
   ...
   * Step number: 2 Step Name: Step Description: - Click the notification Input
   * Data: Expected Outcome: - The user is redirected to the activity viewer with
   * all comment expanded. - The comment where the mention has been done is
   * highlighted
   */
  @Test
   ...
   */
  /*
   * Step number: 2 Step Name: Step Description: - Go to View All page Input Data:
   * Expected Outcome: - The Mention notification is displayed / available in the
   * page
   */
  @Test
