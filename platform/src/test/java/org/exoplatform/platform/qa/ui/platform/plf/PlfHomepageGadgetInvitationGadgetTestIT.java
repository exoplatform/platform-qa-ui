...
public class PlfHomepageGadgetInvitationGadgetTestIT extends Base {
  ...
  public void test03_AcceptARequest() {
    ...
    $(byXpath("//*[@id='ToolBarSearch']//*[@class='dropdown-toggle']")).waitUntil(Condition.visible,Configuration.openBrowserTimeoutMs).click();
    ELEMENT_GADGET_INVITATION.find(byText(username2 + " " + username2)).parent().hover();
    ELEMENT_GADGET_INVITATION.find(byText(username2 + " " + username2)).parent().parent().find(ELEMENT_BUTTON_CONNECT_USER_FROM_GADGET).waitUntil(Condition.visible, Configuration.openBrowserTimeoutMs).click();
