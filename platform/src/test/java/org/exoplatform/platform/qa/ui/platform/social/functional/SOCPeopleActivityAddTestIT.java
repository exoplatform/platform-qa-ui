...
public class SOCPeopleActivityAddTestIT extends Base {
    ...
    public void test01_AddAShareLink() {
        ...
        $(byXpath(ELEMENT_ACTIVITY_TITLE.replace("${text}", textDes).replace("${file}", link))).waitUntil(Condition.visible, Configuration.timeout);
        manageLogInOut.signIn(username2, password);
        homePagePlatform.goToConnections();
    ...
    public void test03_AddNewYourActivity() {
        ...
        homePagePlatform.goToHomePage();
        $(byXpath(ELEMENT_ACTIVITY_AUTHOR_ACTIVITY.replace("${activityText}", activity1))).waitUntil(Condition.visible, Configuration.timeout);
        manageLogInOut.signIn(DATA_USER1, "gtngtn");
        navigationToolbar.goToManageCommunity();
        addUsers.deleteUser(username1);
        addUsers.deleteUser(username2);
    ...
    public void test04_CloseShareLink() {
        ...
        $(byXpath(ELEMENT_ACTIVITY_TITLE.replace("${text}", textDes).replace("${file}", link))).waitUntil(Condition.not(Condition.visible), Configuration.timeout);
        manageLogInOut.signIn(DATA_USER1, "gtngtn");
        navigationToolbar.goToManageCommunity();
        addUsers.deleteUser(username1);
    }
