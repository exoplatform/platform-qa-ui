...
public class GateinGlobalSettingsTestIT extends Base{
	...
	public void test02_ChangeUserProfile(){
		...
		manageLogInOut.signInCas(username, password);
		//waitForAndGetElement(ELEMENT_NAME_OF_USER_TOP_RIGHT.replace("${firstName}", username).replace("${lastName}", username));
		info("Change User Profile");
		...
		manageLogInOut.signIn(username, password);
		//waitForAndGetElement(ELEMENT_NAME_OF_USER_TOP_RIGHT.replace("${firstName}", newFirstName).replace("${lastName}", newLastName));
