...
public class WikiValidattions {
    ...
    public void verifyEditPermisison(String userGroupMembership, boolean isChecked) {
        if (isChecked) {
            ...
            info("Verify that edit permission is checked");
            $(byXpath(ELEMENT_PERMISSION_EDIT_USER_CHECKED
                    .replace("$userGroup", userGroupMembership))).waitUntil(Condition.visible, Configuration.timeout);
        } else {
            info("Verify that edit permission isnot checked");
            $(byId("EDITPAGE" + userGroupMembership)).shouldNotBe(Condition.checked);
        }
    }
    ...
    public void verifyViewPermisison(String userGroupMembership, boolean isChecked) {
        if (isChecked) {
            ...
            $(byXpath(ELEMENT_PERMISSION_VIEW_USER_CHECKED.replace("$userGroup", userGroupMembership))).waitUntil(Condition.not(Condition.visible), Configuration.timeout);
        } else {
