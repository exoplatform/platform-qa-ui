...
public class TribeWikiValidattions {
    ...
    public TribeWikiValidattions(TestBase testBase) {
    ...
    public void verifyCompareVersions(String oldVersion) {
        ...
        evt.waitForAndGetElement(ELEMENT_WIKI_TRIBE_COMPARE_VERSION_TITLE);
        info("Verify that Version N-1 and current version is shown on the page");
        $(byXpath(ELEMENT_COMPARE_TRIBE_VERSION_NUMBER.replace("$num", oldVersion))).waitUntil(Condition.visible,Configuration.openBrowserTimeoutMs + Configuration.collectionsTimeout);
        evt.waitForAndGetElement(ELEMENT_COMPARE_TRIBE_CURRENT_VERSION);
    }
