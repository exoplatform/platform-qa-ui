...
public class TribeSpaceManagement {
  ...
  public void inviteUsersToJoinSpaceViaSpaceMembersTribe(ArrayList<String> groups, int... params) {

    sleep(1000);

    ELEMENT_INVITE_USER_SPACE_MEMBERS_DW.waitUntil(Condition.visible,Configuration.openBrowserTimeoutMs).click();
    ELEMENT_INVITE_USERS_SPACE_MEMBERS_TRIBE.waitUntil(Condition.visible,Configuration.openBrowserTimeoutMs);
    if (groups!= null) {
      for (int i = 0; i < groups.size(); i++) {
        sleep(Configuration.timeout);
        ELEMENT_SPACE_INPUT_USER_TRIBE.waitUntil(Condition.visible, Configuration.openBrowserTimeoutMs).click();
