...
public class TribeWikiPageInformation {
   ...
   */
  public TribeWikiPageInformation(TestBase testBase) {
  ...
  public void addRelations(String location, String page) {
    ...
    $(ELEMENT_ADD_RELATED_TRIBE_SLECTION).waitUntil(Condition.visible,Configuration.timeout).click();
    if(!location.isEmpty()){
      info("Select a location");
      $(byXpath(ELEMENT_ADD_RELATED_POPUP_TRIBE_LOCATION
              .replace("${location}",location))).waitUntil(Condition.visible,Configuration.timeout).click();
    }
    if(!page.isEmpty()){
      info("Select a page in the list");
      $(byXpath(ELEMENT_ADD_RELATED_POPUP_TRIBE_CONTENT
    ...
    public void goToAddRelations() {
        ...
        $(ELEMENT_TRIBE_INFO_ADD_MORE_RELATIONS).waitUntil(Condition.visible, Configuration.openBrowserTimeoutMs).click();
        $(ELEMENT_ADD_RELATED_TRIBE_POPUP_DROPDOWN).waitUntil(Condition.appears, Configuration.openBrowserTimeoutMs);
