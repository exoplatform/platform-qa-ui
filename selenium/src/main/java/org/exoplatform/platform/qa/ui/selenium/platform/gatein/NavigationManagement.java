...
public class NavigationManagement {
  ...
  public void selectItem(specifiContextMenu link) {
    ...
    case EDIT_NODE_PAGE:
      ...
      evt.click(ELEMENT_MANAGESITES_CONTEXTMENU_EDIT_NODE_PAGE_ICON);
      Utils.pause(2000);
      break;
    case EDIT_THIS_NODE:
      info("Click on Edit icon");
      evt.click(ELEMENT_MANAGESITES_CONTEXTMENU_EDIT_ICON);
      Utils.pause(2000);
      break;
    case COPY_NODE:
    ...
    case DELETE_NODE:
      ...
      evt.click(ELEMENT_MANAGESITES_CONTEXTMENU_DELETE_ICON);
      alert.acceptAlert();
      break;
    case MOVE_UP:
  ...
  public void deleteNode(String title) {
    ...
    evt.rightClickOnElement(ELEMENT_NAVIGATION_MANAGEMENT_NODE_NAME.replace("${name}", title));
    info("Select Delete link");
    selectItem(specifiContextMenu.DELETE_NODE);
    info("Verify that the node is deleted");
    evt.waitForElementNotPresent(ELEMENT_NAVIGATION_MANAGEMENT_NODE_NAME.replace("${name}", title));
    evt.waitForAndGetElement(ELEMENT_NAVIGATION_MANAGEMENT_SAVE);
    info("Click on Save button");
    evt.click(ELEMENT_NAVIGATION_MANAGEMENT_SAVE);
    Utils.pause(2000);
  }
  ...
  public void inputInfoNodeSetting(boolean isNotLabelMode,
    ...
    if (isNotLabelMode == true) {
      ...
      evt.uncheck(ELEMENT_NODE_SETTING_SWITCH_MODE, 2);
      if (!label.isEmpty()) {
        info("Input a label");
        evt.type(ELEMENT_NODE_SETTING_LABEL_FIELD_2, label, true);
      }
    } else {
      if (!language.isEmpty()) {
        info("Click on Language box");
        evt.select(ELEMENT_NODE_SETTING_LANGUAGE_BOX, language);
      }
      if (!label.isEmpty()) {
        info("Input a label");
        evt.type(ELEMENT_NODE_SETTING_LABEL_FIELD_1, label, true);
      }
    }

    if (isVisibale == true) {
      info("Uncheck visible box");
      evt.uncheck(ELEMENT_NODE_SETTING_VISIBLE, 2);
    } else {
      if (isPublishDateTime == true) {
        info("Check publish date and time box");
        evt.check(ELEMENT_NODE_SETTING_PUBLISH_DATE_TIME, 2);
      }
    }
  }
