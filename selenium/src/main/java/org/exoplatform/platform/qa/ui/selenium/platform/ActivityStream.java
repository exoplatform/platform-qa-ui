...
public class ActivityStream  {
	...
	public void checkActivity(String name){
		...
		evt.waitForAndGetElement(By.xpath(ELEMENT_ACTIVITY_ELEMENT_IN_ACTIVITY_STREAM.replace("${title}",name)),testBase.getDefaultTimeout(),1);
	...
	public void checkNotShownActivity(String name){
		...
		evt.waitForElementNotPresent(ELEMENT_ACTIVITY_ELEMENT_IN_ACTIVITY_STREAM.replace("${title}",name),testBase.getDefaultTimeout(),1);
	...
	public void checkCommentOfActivity(String activity, String comment){
		...
		evt.waitForAndGetElement(ELEMENT_ACTIVITY_COMMENT
	...
	public void checkActivityAddWebContent(String title, String version, String status){
		...
		evt.waitForAndGetElement(By.xpath(ELEMENT_ACTIVITY_WEBCONTENT_CHECK_VERSION.replace("${title}", title).replace("{$version}", version)));
		evt.waitForAndGetElement(By.xpath(ELEMENT_ACTIVITY_WEBCONTENT_CHECK_STATUS.replace("${title}", title).replace("{$status}", status)));
	}
	...
	public void checkActivityAddProduct(String title, String version, String status){
		...
		evt.waitForAndGetElement(By.xpath(ELEMENT_ACTIVITY_PRODUCT_CHECK_VERSION.replace("{$title}", title).replace("{$version}", version)));
		evt.waitForAndGetElement(By.xpath(ELEMENT_ACTIVITY_PRODUCT_CHECK_STATUS.replace("{$title}", title).replace("{$status}", status)));
	}
	...
	public void addComment(String filename, String textContent){
		...
		int repeat1 = 0;
		while(evt.waitForAndGetElement(ELEMENT_PUBLICATION_COMMENTPOSTED
				.replace("${content}", textContent)
				.replace("$activity",filename),2000,0)==null){
			if (repeat1 > 5)
				break;
			if(evt.waitForAndGetElement(ELEMENT_PUBLICATION_COMMENTPOSTED
					.replace("${content}", textContent)
					.replace("$activity",filename),2000,0)!=null)
				break;
			else{
				evt.switchToParentWindow();
			WebElement input = evt.waitForAndGetElement(ELEMENT_COMMENTBOX.replace(
					"${title}", filename));
	...
	public void addCommentUsingJavascript(String activityText, String contentOfComment){
		...
		((JavascriptExecutor)testBase.getSeleniumDriver()).executeScript("arguments[0].textContent = '"+contentOfComment+"';", commentText);
		((JavascriptExecutor)testBase.getSeleniumDriver()).executeScript("arguments[0].disabled = false;", commentButton);
		((JavascriptExecutor)testBase.getSeleniumDriver()).executeScript("arguments[0].className = 'btn pull-right btn-primary';", commentButton);
		evt.click(ELEMENT_COMMENT_BUTTON.replace("${activityText}", activityText));
		info("Verify comment successfully");
		evt.waitForAndGetElement(ELEMENT_DELETE_COMMENT_BUTTON.
				replace("${activityText}", activityText).
				replace("${commentText}", contentOfComment), testBase.getDefaultTimeout(),1,2);
	...
	public void deleteComment(String name,String comment){
		...
		evt.waitForElementNotPresent(ELEMENT_PUBLICATION_COMMENTPOSTED
				.replace("$activity",name)
	...
	public void addText(String text){
		...
		((JavascriptExecutor)testBase.getSeleniumDriver()).executeScript("arguments[0].className = 'pull-right btn btn-primary';", shareButton);
	}
	...
	public void shareActivity(){
		...
		else evt.click(ELEMENT_COMPOSER_SHARE_BUTTON);
		Utils.pause(1000);
	...
	public void addCommentWithMentionUser(String activity, String username, String textContent) throws AWTException{
		for(int repeat=0;repeat<5;repeat++){
			...
			if(evt.waitForAndGetElement(ELEMENT_PUBLICATION_COMMENTPOSTED_MENTION.replace(
		...
		evt.waitForAndGetElement(ELEMENT_PUBLICATION_COMMENTPOSTED_MENTION.replace(
	...
	public void likeActivity(String activityText){
		...
		info("-- Like activity --");
		int numLike = Integer.parseInt(evt.waitForAndGetElement(ELEMENT_LIKE_NUMBER.replace("${title}", activityText)).getText().trim());
		...
		int newNumLike = Integer.parseInt(evt.waitForAndGetElement(ELEMENT_LIKE_NUMBER.replace("${title}", activityText)).getText().trim());
	...
	public void checkFormatComment(String activity,String comment,String fullName){
		...
		evt.waitElementAndTryGetElement(ELEMENT_COMMENT_AVATAR_USER
				.replace("$activity",activity)
				.replace("$comment",comment)
				.replace("$fullName",fullName));
		info("Name of user comment");
		evt.waitElementAndTryGetElement(ELEMENT_COMMENT_AUTHOR
				.replace("$activity",activity)
				.replace("$comment",comment)
				.replace("$fullName",fullName));
		info("Time comment is posted");
		evt.waitElementAndTryGetElement(ELMEMENT_COMMENT_TIME
				.replace("$activity",activity)
				.replace("$comment",comment));
	}
	/**
