...
casper.then( function(){ 
    casper.echo('create spacefor forum');
...
casper.thenOpen(scenario.url + "/portal/g/:spaces:test22/test22", function() {
...
casper.thenOpen(scenario.url + "/portal/g/:spaces:test22/test22/forum", function() {
    casper.echo('Clicking forum button');
    casper.waitForSelector('div.UIForumContainer');
    casper.wait(4000);
    casper.echo('newSpace forum');
    this.capture('C:/Backstopjs/Backstopjs Projects/DemoExo/backstop_data/user-action-screenshots/' + vp.name+'.png');
